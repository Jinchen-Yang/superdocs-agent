...
export function AdminModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <Dialog
      label="管理统计"
      onClose={onClose}
      className="panel fixed left-1/2 top-1/2 z-50 flex max-h-[88vh] w-[94%] max-w-[520px] -translate-x-1/2 -translate-y-1/2 flex-col gap-2.5 overflow-y-auto rounded-3xl p-5"
    >
        <div className="flex items-center gap-3">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5 text-[17px] font-semibold"><BarChart3 className="size-4.5 text-accent" /> 管理统计</div>
            <div className="text-sub text-[12.5px]">superdocs · 仅管理员可见</div>
          </div>
        ...
        {err && <div className="text-[13px] font-medium text-[var(--danger)]">{err}</div>}
        ...
        {s && (
          <>
            ...
            {s.byModel.map((m) => (
              <div key={m.model} className="flex justify-between rounded-lg bg-[var(--surface-2)] px-3 py-1.5 text-[12.5px]">
                <span className="font-medium">{m.model}</span>
                <span className="text-sub font-medium">{fmt(m.tokens)} tk · {m.calls} 次</span>
              </div>
            ))}
            ...
            {s.topUsers.map((t, i) => (
              <div key={i} className="flex items-center gap-2 rounded-lg bg-[var(--surface-2)] px-3 py-1.5 text-[12.5px]">
                <span className="text-faint w-5 font-medium">{i + 1}</span>
                <span className="min-w-0 flex-1 truncate font-medium">{t.name}{t.sid && <span className="text-faint font-normal"> · {t.sid}</span>}</span>
                <span className="text-sub shrink-0 font-medium">{fmt(t.tokens)} tk · {t.calls} 次</span>
              </div>
            ))}
          </>
        )}
