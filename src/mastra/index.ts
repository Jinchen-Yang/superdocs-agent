...
// 提为具名实例并给其 pool 挂 error 监听（同 memory.ts，避免空闲连接错误 crash 进程）。
const mastraStorage = new PostgresStore({ id: 'superdocs', connectionString: process.env.DATABASE_URL! });
attachPgPoolErrorHandler((mastraStorage as any).pool, 'mastra-storage');
